...
## De opdracht
- Probeer nogmaals het `hydra` commando uit dat je bij `low` hebt uitgevonden, maar pas `security=low` in het commando
aan naar `security=medium`. Wat valt je op?

> ℹ️ **Tip** Je kunt de duur van een linux commando meten door er `time` voor te zetten:
> `time hydra ...`
